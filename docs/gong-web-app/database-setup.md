...
def init_data(db):
    ...
    """).strip('\n')
    if not centers():
        centers.insert(center_name="Mahi", gong_db_name="mahi.ok.db", location="1396", timezone="Europe/Paris", other_course=oc_mahi,  status="free", created_by="", status_start="2026-01-01", json_save="")
        centers.insert(center_name="Pajjota", gong_db_name="pajjota.ok.db", location="1370", timezone="Europe/Brussels", other_course=oc_pajj, status="free", created_by="", status_start="2026-01-01", json_save="")
    else:
        centers.update(center_name="Mahi", other_course=oc_mahi)
        centers.update(center_name="Pajjota", other_course=oc_pajj)
