...
| Color | Following text | Explanation | Todo |
| -------- | -------- | --------- | -----|
| no |  | OK for the gong planning | nothing |
| no | Time overlap | time overlap on same day from a variable period | nothing |
| <span style="background-color:orange">orange</span> | CHECK Time overlap | time operlap on same day with another non variable period | check for conflict |
| <span style="background-color:orange">orange</span> | CHECK Overlap of X days | days operlap fron a variable period | check for dhamma.org conflict: change plan plus report if needed |
| <span style="background-color:orange">orange</span> | CHECK GAP X days | A gap in the plan will be filled with the 'default period' | should be OK, just check |
...
| <span style="background-color:red">--red--</span> | Same starting time | In the gong plan, 2 periods cannot have the same starting time | delete or change at least one starting time |
| <span style="background-color:red">--red--</span> | Overlap of X days | days overlap from a fixed period | dhamma.org conflict: change plan plus report |
